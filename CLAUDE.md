...
1. **Migrations are forward-only.** Never edit a `0NN_*.sql` file after it's been applied. Add a new one instead. Current versions: `001–083` (some gaps are fine; every prefix must be unique).
...
backend/
  ...
  migrations/         Forward-only sqlx migrations (001–083)
