...
agents covering frontend, backend, ops). **Bar:** _ship-ready at a Meta
principal-engineer bar_ — security, correctness, performance, and operability.
...
2. **Webhook handlers return `StatusCode` not Problem+JSON**
   ...
   _need_ Problem+JSON, but the inconsistency makes incident response harder.
