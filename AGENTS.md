...
| -------- | -------------------------------------------------- | --------- |
| Frontend | SvelteKit · Svelte 5 (runes) · TailwindCSS v4      | Vercel    |
...
1. **Migrations are forward-only.** Once a `0NN_*.sql` file has been
   applied to *any* environment, **never** edit its contents. sqlx
...
3. **Migration ordering matters.** A migration that does
   `REFERENCES other_table(id)` must come *after* the one that creates
...
5. **Permission-check every admin mutation.** Admin handlers must call
   `policy.require(ctx, "<perm>")` *before* doing the work. The full
...
| Backend dev server                | `pnpm dev:api`  *(runs `cargo run` inside `backend/`)*           |
...
* When you add a new worker or middleware, add the metric *and* the
  alert *and* the runbook section *in the same change*. CI does not
