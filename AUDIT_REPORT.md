...
- **Why it matters:** the SPA uses cookies (no Bearer) → even when frontend *does* send `Idempotency-Key`, middleware silently bypasses caching. Compare to `extractors.rs:123-155` (`extract_access_token`) which correctly reads cookie OR Bearer.
