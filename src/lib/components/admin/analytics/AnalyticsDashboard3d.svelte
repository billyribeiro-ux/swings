	...
	onMount(() => {
		...
		void tick().then(() => {
			...
			if (elTime && summary.time_series.length) {
				const values = summary.time_series.map((t) => t.page_views);
				const c = mountBarChart(elTime, values, 0x3b82f6);
				if (cancelled) c();
...
<div class="dash3d">
	{#if summary.time_series.length}
		<section class="dash3d__block">
			<h3 class="dash3d__title">Page views over time</h3>
			<p class="dash3d__hint">Daily page views (3D bars)</p>
