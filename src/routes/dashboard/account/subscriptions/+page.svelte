						...
						<tr>
							...
							<td>
								...
								{#if sub.cancel_at_period_end}
									<span class="subs__hint"
										>Cancels {formatDate(sub.current_period_end)}</span
									>
								{/if}
							</td>
							<td class="subs__cell-product">{planLabel(sub)}</td>
							<td class="subs__cell-date">{nextPaymentLabel(sub)}</td>
							<td class="subs__cell-num"
								>{formatMoney(sub.amount_cents, sub.currency)}</td
							>
...
<style>
	...
	.subs__table tbody tr:last-child th,
	.subs__table tbody tr:last-child td {
		border-bottom: none;
	}
	.subs__table thead th {
		font-size: var(--fs-xs);
