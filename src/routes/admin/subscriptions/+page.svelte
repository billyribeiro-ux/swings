...
<svelte:head>
	<title>Subscriptions - Admin - Explosive Swings</title>
</svelte:head>
		...
		<div class="subs-page__kpis">
			...
			<div class="kpi">
				<div class="kpi__icon kpi__icon--rose">
					<TrendDown size={22} weight="fill" />
				</div>
				<div>
					<p class="kpi__label">Churn Rate</p>
					<p class="kpi__value">{stats.churn_rate.toFixed(1)}%</p>
				</div>
			</div>
		</div>
	{/if}
				...
				>
					...
					<div class="sub-card__details">
						<div class="sub-card__row">
							...
							<span class="sub-card__value">{sub.plan_name}</span>
						</div>
						<div class="sub-card__row">
				...
				<tbody>
					{#each subscriptions as sub (sub.id)}
						...
						>
							...
							<td class="s-table__amount">{formatMoney(sub.amount_cents)}<span class="s-table__interval">/{sub.interval === 'month' ? 'mo' : 'yr'}</span></td>
...
<style>
	...
	.subs-page__search:focus-within {
	...
	.subs-page__search-input::placeholder {
		color: var(--color-grey-500);
	}

	.subs-page__filter-group {
		display: flex;
		align-items: center;
		gap: 0.5rem;
		color: var(--color-grey-400);
	...
	.sub-card__status--canceled {
		background-color: rgba(255, 255, 255, 0.06);
		color: var(--color-grey-400);
	}

	.sub-card__status--past_due {
	...
	.subs-page__page-btn:hover:not(:disabled) {
		border-color: var(--color-teal);
	}

	.subs-page__page-btn:disabled {
		opacity: 0.4;
		cursor: not-allowed;
	}

	.subs-page__page-info {
		font-size: var(--fs-xs);
		color: var(--color-grey-400);
	}
	...
	@media (min-width: 768px) {
		...
		.s-table__status--active {
			background-color: rgba(34, 181, 115, 0.12);
			color: var(--color-green);
		}

		.s-table__status--canceled {
			background-color: rgba(255, 255, 255, 0.06);
			color: var(--color-grey-400);
		}

		.s-table__status--past_due {
			background-color: rgba(224, 72, 72, 0.12);
			color: var(--color-red);
		}
