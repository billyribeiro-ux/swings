<script lang="ts">
	...
	onMount(async () => {
		try {
			const p = await api.get<
				Popup & { impressions?: number; submissions?: number; conversion_rate?: number }
	...
	function removePage(p: string) {
		pages_ = pages_.filter((x) => x !== p);
	}
	function addPage() {
		if (newPage.trim()) {
			pages_ = [...pages_, newPage.trim()];
			newPage = '';
		}
	}
	function toggleDevice(d: 'desktop' | 'mobile' | 'tablet') {
		devices = devices.includes(d)
			? (devices.filter((x) => x !== d) as typeof devices)
			: [...devices, d];
	}
	...
	async function handleSave() {
		...
		try {
			...
			error = e instanceof ApiError ? e.message : 'Failed to update';
		} finally {
			saving = false;
		}
	}
	...
	{:else}
		...
		{#if analytics}
			<div class="popup-edit__analytics">
				<div class="stat">
					<span class="stat__value">{analytics.total_impressions}</span><span
						class="stat__label">Impressions</span
					>
				</div>
				<div class="stat">
					<span class="stat__value">{analytics.total_submissions}</span><span
						class="stat__label">Submissions</span
					>
				</div>
				<div class="stat">
					<span class="stat__value">{(analytics.conversion_rate * 100).toFixed(1)}%</span
		...
		<div class="popup-edit__config">
			{#if activeTab === 'trigger'}
				<label class="field"
					...
					<select bind:value={triggerType} class="field__input">
						<option value="on_load">On Load</option><option value="exit_intent"
							>Exit Intent</option
						>
						<option value="scroll_percentage">Scroll %</option><option
							value="time_delay">Time Delay</option
						>
						<option value="click">Click</option><option value="inactivity"
							>Inactivity</option
						>
					</select>
				</label>
				...
				{#if triggerType === 'scroll_percentage'}
					<label class="field"
						><span>Scroll %</span><input
							type="range"
							min="0"
							max="100"
							bind:value={triggerScrollPct}
						/><span>{triggerScrollPct}%</span></label
					>
				{/if}
			{:else if activeTab === 'targeting'}
				<div class="field">
					<span>Pages</span>
					{#each pages_ as p (p)}<span class="chip"
							>{p} <button onclick={() => removePage(p)}>&times;</button></span
						>{/each}
					<div class="field__row">
						<input
							...
							type="text"
							aria-label="Add page pattern"
							bind:value={newPage}
							placeholder="/blog/*"
							class="field__input"
						/><button class="field__btn" onclick={addPage} aria-label="Add page"
				...
				<div class="field">
					<span>Devices</span>
					{#each ['desktop', 'mobile', 'tablet'] as const as d (d)}<label class="checkbox"
							><input
								type="checkbox"
			...
			{:else if activeTab === 'style'}
				<div class="field__grid">
					<label class="field"
						><span>Background</span><input type="color" bind:value={bg} /></label
					>
					<label class="field"
						><span>Text</span><input type="color" bind:value={textColor} /></label
					>
					<label class="field"
						><span>Accent</span><input type="color" bind:value={accentColor} /></label
					>
					<label class="field"
					...
					<label class="field"
						...
						<select bind:value={animation} class="field__input">
							{#each ['fade', 'slide_up', 'slide_down', 'scale', 'none'] as a (a)}<option
									value={a}>{a}</option
								>{/each}
						</select>
					</label>
			...
			{:else if activeTab === 'display'}
				<label class="field"
					...
					<select bind:value={frequency} class="field__input">
						<option value="every_time">Every Time</option><option
							value="once_per_session">Once Per Session</option
						>
		...
		<div class="popup-edit__elements">
			...
			<div class="popup-edit__add-bar">
				{#each [['heading', 'H'], ['text', 'T'], ['email', '@'], ['input', 'Aa'], ['button', 'Btn'], ['divider', '—']] as [type, label] (type)}
					<button class="add-btn" onclick={() => addElement(type as PopupElement['type'])}
						><PlusIcon size={12} /> {label}</button
					>
				{/each}
			</div>
			{#each elements as el, i (el.id)}
				<div class="element" class:element--editing={editingId === el.id}>
					<div class="element__row">
						...
						<span class="element__preview"
							>{String(el.props.text ?? el.props.label ?? '')}</span
						>
						<button onclick={() => (editingId = editingId === el.id ? null : el.id)}
							...
							{#if el.type === 'email' || el.type === 'input'}
								<label class="field"
									><span>Label</span><input
										type="text"
										bind:value={el.props.label}
										class="field__input"
									/></label
								>
								<label class="field"
									><span>Placeholder</span><input
										type="text"
										bind:value={el.props.placeholder}
										class="field__input"
									/></label
								>
							{/if}
		...
		<div class="popup-edit__preview">
			...
			<div
				class="preview-box"
				style="background:{bg}; color:{textColor}; border-radius:{borderRadius}; max-width:{maxWidth}; padding:1.5rem;"
			>
				{#each elements as el (el.id)}
					{#if el.type === 'heading'}<h2 style="margin:0 0 0.5rem">
							{el.props.text ?? 'Heading'}
						</h2>
					{:else if el.type === 'text'}<p
							style="margin:0 0 0.5rem; opacity:0.8; font-size:0.9rem"
						>
							{el.props.text ?? ''}
						</p>
					{:else if el.type === 'email' || el.type === 'input'}<div
						...
						>
							...
							><input
								...
								style="width:100%; padding:0.5rem; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); border-radius:6px; color:{textColor}"
							/>
						</div>
					{:else if el.type === 'button'}<button
							style="padding:0.6rem 1.5rem; background:{accentColor}; color:white; border:none; border-radius:6px; font-weight:600; cursor:pointer; width:100%"
							>{el.props.text ?? 'Submit'}</button
						>
					{:else if el.type === 'divider'}<hr
							style="border:none; border-top:1px solid rgba(255,255,255,0.1); margin:0.75rem 0"
						/>
					{/if}
				{/each}
			</div>
...
<style>
	...
	.popup-edit__elements {
		margin-bottom: 1.5rem;
	}
	.popup-edit__elements h3 {
		font-size: var(--fs-base);
		font-weight: var(--w-semibold);
		color: var(--color-white);
		margin-bottom: 0.75rem;
	...
	.element__row button:hover {
		color: var(--color-white);
	}
	.element__edit {
		padding: 0.75rem 0 0;
	}
	.popup-edit__preview h3 {
		display: flex;
		align-items: center;
		gap: 0.5rem;
		font-size: var(--fs-base);
