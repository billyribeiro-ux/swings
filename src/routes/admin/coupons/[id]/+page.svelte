		...
		<div class="ce__stats">
			...
			<div class="ce__sbar">
				...
				{#if coupon.usage_limit != null}
					<div class="ce__barbg"><div class="ce__barfill" style="width: {usagePercent}%"></div></div>
				{/if}
			</div>
					...
					<div class="ce__ulist">
						{#each coupon.recent_usages as usage (usage.user_email + usage.used_at)}
							<div class="ce__urow">
								...
								<span class="ce__udate">{new Date(usage.used_at).toLocaleDateString()}</span>
							</div>
						{/each}
					</div>
				</div>
			...
			<div class="ce__cols">
				<div class="ce__col">
					<div class="ce__f">
						...
						<input id="code" type="text" value={coupon.code} readonly class="ce__inp ce__inp--ro" />
					</div>
					<div class="ce__f">
						<label for="description" class="ce__lbl">Description</label>
						<textarea id="description" bind:value={description} class="ce__ta" rows="3" placeholder="Internal note about this coupon..."></textarea>
					</div>
					<div class="ce__f">
					...
					<div class="ce__f">
						<label for="value" class="ce__lbl">Value {discountType === 'percentage' ? '(%)' : discountType === 'fixed' ? '($)' : '(days)'}</label>
						<input id="value" type="number" step="any" min="0" bind:value={value} required class="ce__inp" />
					</div>
					<div class="ce__f">
						<label for="minPurchase" class="ce__lbl">Min Purchase ($)</label>
						<input id="minPurchase" type="number" step="0.01" min="0" bind:value={minPurchase} class="ce__inp" placeholder="0.00" />
					</div>
					<div class="ce__f">
						<label for="maxDiscount" class="ce__lbl">Max Discount ($)</label>
						<input id="maxDiscount" type="number" step="0.01" min="0" bind:value={maxDiscount} class="ce__inp" placeholder="No limit" />
					</div>
				</div>
				<div class="ce__col">
					<div class="ce__f">
						<label for="usageLimit" class="ce__lbl">Usage Limit</label>
						<input id="usageLimit" type="number" min="0" bind:value={usageLimit} class="ce__inp" placeholder="Unlimited" />
					</div>
					<div class="ce__f">
						<label for="perUserLimit" class="ce__lbl">Per-User Limit</label>
						<input id="perUserLimit" type="number" min="0" bind:value={perUserLimit} class="ce__inp" placeholder="Unlimited" />
					</div>
					<div class="ce__f">
					...
					<div class="ce__f">
						...
						<input id="expiryDate" type="date" bind:value={expiryDate} class="ce__inp" />
					</div>
					<label class="ce__tog"><input type="checkbox" bind:checked={stackable} /><span class="ce__track"><span class="ce__thumb"></span></span><span>Stackable</span></label>
					<label class="ce__tog"><input type="checkbox" bind:checked={firstPurchaseOnly} /><span class="ce__track"><span class="ce__thumb"></span></span><span>First Purchase Only</span></label>
					<label class="ce__tog"><input type="checkbox" bind:checked={active} /><span class="ce__track"><span class="ce__thumb"></span></span><span>Active</span></label>
				</div>
			</div>
			<div class="ce__acts">
...
<style>
	...
	.ce__barbg { width: 100%; height: 0.5rem; background-color: rgba(255,255,255,0.08); border-radius: var(--radius-full); overflow: hidden; }
	.ce__barfill { height: 100%; background: linear-gradient(90deg, var(--color-teal), var(--color-teal-light)); border-radius: var(--radius-full); transition: width 300ms var(--ease-out); }
	.ce__usages { margin-top: 1.25rem; }
	.ce__ulbl { font-size: var(--fs-xs); font-weight: var(--w-semibold); color: var(--color-grey-400); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.75rem; }
	.ce__ulist { display: flex; flex-direction: column; gap: 0.35rem; }
	.ce__urow { display: flex; align-items: center; gap: 1rem; padding: 0.5rem 0.75rem; background-color: rgba(255,255,255,0.03); border-radius: var(--radius-md); font-size: var(--fs-sm); }
	...
	.ce__inp--ro { opacity: 0.6; cursor: not-allowed; border-style: dashed; }
	.ce__tog { display: flex; align-items: center; gap: 0.75rem; font-size: var(--fs-sm); color: var(--color-grey-300); cursor: pointer; padding: 0.4rem 0; }
	.ce__tog input[type='checkbox'] { position: absolute; opacity: 0; width: 0; height: 0; }
	.ce__track { position: relative; width: 2.5rem; height: 1.35rem; background-color: rgba(255,255,255,0.1); border-radius: var(--radius-full); flex-shrink: 0; transition: background-color 200ms var(--ease-out); }
	.ce__tog input:checked + .ce__track { background-color: var(--color-teal); }
	.ce__thumb { position: absolute; top: 0.15rem; left: 0.15rem; width: 1.05rem; height: 1.05rem; background-color: var(--color-white); border-radius: var(--radius-full); transition: transform 200ms var(--ease-out); }
