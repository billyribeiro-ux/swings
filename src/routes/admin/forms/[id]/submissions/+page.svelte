...
<script lang="ts">
	...
	async function refresh() {
		...
		try {
			...
			error =
				e instanceof ApiError ? `${e.status}: ${e.message}` : 'Failed to load submissions';
		} finally {
			loading = false;
		}
	}
	...
	async function runBulk() {
		...
		if (bulkAction === 'delete') {
			const ok = await confirmDialog({
				...
				message:
					'The submissions and any attached payload will be removed. This cannot be undone.',
	...
	async function rowAction(row: SubmissionRow, action: BulkAction) {
		...
		if (action === 'delete') {
			const ok = await confirmDialog({
				...
				message:
					'The submission and any attached payload will be removed. This cannot be undone.',
				confirmLabel: 'Delete permanently',
				variant: 'danger'
			});
			if (!ok) return;
	...
	{#if selected}
		...
		<aside class="drawer" aria-label="Submission detail">
			...
			<dl class="meta">
				...
				<dt>IP hash</dt>
				<dd><code>{selected.ip_hash}</code></dd>
				<dt>User agent</dt>
				<dd>{selected.user_agent || '—'}</dd>
				{#if selected.referrer}
					<dt>Referrer</dt>
					<dd>{selected.referrer}</dd>
				{/if}
			</dl>
			<details open>
