<script lang="ts">
	...
	interface Course { id: string; title: string; slug: string; description: string; difficulty: 'beginner' | 'intermediate' | 'advanced'; price: number; is_free: boolean; thumbnail_url: string | null; is_published: boolean; modules: Module[]; }
	...
	onMount(async () => {
		...
		} catch { error = 'Course not found'; } finally { loading = false; }
	});
	...
	function updateLesson(mi: number, li: number, field: keyof Lesson, val: string | boolean) {
		modules = modules.map((m, i) => i !== mi ? m : { ...m, lessons: m.lessons.map((l, j) => j === li ? { ...l, [field]: val } : l) });
	}
	...
	async function handleSave(e: Event) {
		...
		try {
			const payload = {
				title: title.trim(), slug: slug.trim(), description: description.trim(), difficulty,
				price: Number(price) || 0, thumbnail_url: thumbnailUrl.trim() || null,
				modules: modules.map((m, mi) => ({
					id: m.id.startsWith('new-') ? undefined : m.id, title: m.title, sort_order: mi,
					lessons: m.lessons.map((l, li) => ({
						id: l.id.startsWith('new-') ? undefined : l.id,
						title: l.title, video_url: l.video_url, is_preview: l.is_preview, sort_order: li
					}))
				}))
			};
			const updated = await api.put<Course>(`/api/admin/courses/${page.params.id}`, payload);
			course = updated; modules = updated.modules ?? [];
			...
			<div class="ce__card">
				...
				<div class="ce__grid">
					<div class="ce__f">
						...
						<input id="ce-title" type="text" bind:value={title} required class="ce__inp" placeholder="Course title" />
					</div>
					<div class="ce__f">
						<label for="ce-slug" class="ce__lbl">Slug</label>
						<input id="ce-slug" type="text" bind:value={slug} class="ce__inp ce__inp--mono" placeholder="course-slug" />
					</div>
					<div class="ce__f ce__f--full">
						<label for="ce-desc" class="ce__lbl">Description</label>
						<textarea id="ce-desc" bind:value={description} class="ce__ta" rows="4" placeholder="Course description..."></textarea>
					</div>
					<div class="ce__f">
					...
					<div class="ce__f">
						...
						<input id="ce-price" type="number" step="0.01" min="0" bind:value={price} class="ce__inp" placeholder="0.00" />
					</div>
					<div class="ce__f ce__f--full">
						<label for="ce-thumb" class="ce__lbl">Thumbnail URL</label>
						<input id="ce-thumb" type="url" bind:value={thumbnailUrl} class="ce__inp" placeholder="https://..." />
					</div>
				</div>
				{#if thumbnailUrl}<div class="ce__tp"><img src={thumbnailUrl} alt="Thumbnail" class="ce__timg" /></div>{/if}
			</div>
					...
					{#each modules as mod, mi (mod.id)}
						<div class="ce__mc">
							...
							{#if expandedModules.has(mod.id)}
								<div class="ce__mbody">
									{#if mod.lessons.length === 0}<p class="ce__nol">No lessons in this module.</p>{/if}
...
<style>
	.ce__back { display: inline-flex; align-items: center; gap: 0.5rem; color: var(--color-grey-400); font-size: var(--fs-sm); text-decoration: none; margin-bottom: 1.5rem; transition: color 200ms var(--ease-out); }
	.ce__back:hover { color: var(--color-white); }
	.ce__status { text-align: center; padding: 3rem; color: var(--color-grey-400); }
	.ce__status--err { color: var(--color-red); }
	...
	.ce__form { display: flex; flex-direction: column; gap: 1.5rem; }
	.ce__card { background: rgba(19, 43, 80, 0.35);
		backdrop-filter: blur(24px);
		-webkit-backdrop-filter: blur(24px); border: 1px solid rgba(255,255,255,0.06); border-radius: var(--radius-2xl); padding: 1.5rem; }
	.ce__stitle { font-size: var(--fs-lg); font-weight: var(--w-bold); color: var(--color-white); font-family: var(--font-heading); margin: 0 0 1.25rem; }
	.ce__grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem; }
	.ce__f { display: flex; flex-direction: column; gap: 0.4rem; }
	.ce__f--full { grid-column: 1 / -1; }
	...
	.ce__inp--mono { font-family: 'SF Mono','Fira Code',monospace; font-size: var(--fs-xs); }
	.ce__inp--sm { padding: 0.45rem 0.7rem; font-size: var(--fs-xs); }
	.ce__tp { margin-top: 1rem; border-radius: var(--radius-2xl); overflow: hidden; border: 1px solid rgba(255,255,255,0.08); max-width: 20rem; }
	.ce__timg { width: 100%; height: auto; display: block; max-height: 10rem; object-fit: cover; }
	.ce__mhdr { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.25rem; }
	.ce__mhdr .ce__stitle { margin-bottom: 0; }
	.ce__add { display: inline-flex; align-items: center; gap: 0.35rem; padding: 0.5rem 1rem; background-color: rgba(15,164,175,0.1); border: 1px solid rgba(15,164,175,0.3); border-radius: var(--radius-2xl); color: var(--color-teal); font-size: var(--fs-sm); font-weight: var(--w-semibold); cursor: pointer; transition: background-color 200ms var(--ease-out); }
	.ce__add:hover { background-color: rgba(15,164,175,0.2); }
	.ce__empty { text-align: center; padding: 2rem; color: var(--color-grey-500); font-size: var(--fs-sm); }
	.ce__mlist { display: flex; flex-direction: column; gap: 0.75rem; }
	.ce__mc { border: 1px solid rgba(255,255,255,0.08); border-radius: var(--radius-2xl); overflow: hidden; background-color: rgba(0,0,0,0.15); }
	.ce__mtog { display: flex; align-items: center; gap: 0.75rem; width: 100%; padding: 0.85rem 1rem; background: none; border: none; cursor: pointer; color: var(--color-grey-300); transition: background-color 150ms var(--ease-out); }
	.ce__mtog:hover { background-color: rgba(255,255,255,0.03); }
	.ce__mnum { display: flex; align-items: center; justify-content: center; width: 1.75rem; height: 1.75rem; border-radius: var(--radius-md); background-color: rgba(15,164,175,0.15); color: var(--color-teal-light); font-size: var(--fs-xs); font-weight: var(--w-bold); flex-shrink: 0; }
	.ce__minp { flex: 1; background: transparent; border: 1px solid transparent; border-radius: var(--radius-md); padding: 0.3rem 0.5rem; color: var(--color-white); font-size: var(--fs-sm); font-weight: var(--w-semibold); transition: border-color 200ms var(--ease-out); }
	.ce__minp:focus { outline: none; border-color: var(--color-teal); background-color: rgba(255,255,255,0.05); }
	.ce__minp::placeholder { color: var(--color-grey-500); }
	.ce__mmeta { display: flex; align-items: center; gap: 0.5rem; flex-shrink: 0; }
	.ce__lcnt { font-size: var(--fs-xs); color: var(--color-grey-500); }
	.ce__mbody { padding: 0.75rem 1rem 1rem; border-top: 1px solid rgba(255,255,255,0.06); }
	.ce__nol { color: var(--color-grey-500); font-size: var(--fs-xs); padding: 0.5rem 0; margin: 0; }
	.ce__lr { display: flex; align-items: flex-start; gap: 0.65rem; padding: 0.65rem 0; border-bottom: 1px solid rgba(255,255,255,0.04); }
	.ce__lr:last-of-type { border-bottom: none; }
	.ce__lnum { font-size: var(--fs-xs); color: var(--color-grey-500); font-weight: var(--w-medium); min-width: 2rem; padding-top: 0.5rem; }
	.ce__lf { flex: 1; display: flex; flex-direction: column; gap: 0.4rem; min-width: 0; }
	.ce__lurl { display: flex; align-items: center; gap: 0.4rem; color: var(--color-grey-500); }
	.ce__lurl .ce__inp { flex: 1; }
	.ce__la { display: flex; align-items: center; gap: 0.25rem; padding-top: 0.35rem; flex-shrink: 0; }
	.ce__ib { display: flex; align-items: center; justify-content: center; width: 2rem; height: 2rem; border: none; border-radius: var(--radius-md); background: rgba(255,255,255,0.05); color: var(--color-grey-400); cursor: pointer; transition: all 150ms var(--ease-out); }
	.ce__ib:hover { background: rgba(255,255,255,0.1); color: var(--color-teal-light); }
	.ce__ib--d:hover { background: rgba(239,68,68,0.15); color: var(--color-red); }
	.ce__mft { display: flex; align-items: center; justify-content: space-between; margin-top: 0.75rem; padding-top: 0.75rem; border-top: 1px solid rgba(255,255,255,0.06); }
	.ce__alb { display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.4rem 0.85rem; background: transparent; border: 1px dashed rgba(15,164,175,0.4); border-radius: var(--radius-md); color: var(--color-teal); font-size: var(--fs-xs); font-weight: var(--w-semibold); cursor: pointer; transition: all 150ms var(--ease-out); }
	.ce__alb:hover { background: rgba(15,164,175,0.08); border-color: var(--color-teal); }
	.ce__rmb { display: inline-flex; align-items: center; gap: 0.3rem; padding: 0.4rem 0.85rem; background: transparent; border: 1px solid rgba(239,68,68,0.2); border-radius: var(--radius-md); color: var(--color-grey-500); font-size: var(--fs-xs); cursor: pointer; transition: all 150ms var(--ease-out); }
	.ce__rmb:hover { background: rgba(239,68,68,0.1); color: var(--color-red); border-color: rgba(239,68,68,0.4); }
	.ce__acts { display: flex; align-items: center; justify-content: space-between; background: rgba(19, 43, 80, 0.35);
		backdrop-filter: blur(24px);
		-webkit-backdrop-filter: blur(24px); border: 1px solid rgba(255,255,255,0.06); border-radius: var(--radius-2xl); padding: 1.25rem 1.5rem; }
	...
	.ce__save { display: flex; align-items: center; gap: 0.4rem; padding: 0.6rem 1.5rem; background: linear-gradient(135deg, var(--color-teal), #0d8a94); color: var(--color-white); font-weight: var(--w-semibold); font-size: var(--fs-sm); border-radius: var(--radius-2xl); cursor: pointer; transition: opacity 200ms var(--ease-out); }
	.ce__save:hover:not(:disabled) { opacity: 0.9; }
	.ce__save:disabled { opacity: 0.5; cursor: not-allowed; }
	@media (max-width: 768px) {
		.ce__grid { grid-template-columns: 1fr; }
		.ce__f--full { grid-column: 1; }
		.ce__acts { flex-direction: column; gap: 1rem; }
		.ce__lr { flex-wrap: wrap; }
	}
</style>
