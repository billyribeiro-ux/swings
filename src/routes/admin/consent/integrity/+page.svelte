...
<script lang="ts">
	...
	function fmtWindow(a: IntegrityAnchor): string {
		...
		const start = a.window_start_at ? new Date(a.window_start_at).toLocaleString() : '—';
...
<div class="page" data-testid="admin-consent-integrity">
		...
		<div class="page__title-row">
			...
			<div class="page__copy">
				...
				<p class="page__subtitle">
					Tamper-evidence anchors over <code>consent_records</code>. Each row is a SHA-256
					hash rolled across a window of records — replaying the chain and comparing to
					the anchor proves no record was deleted or rewritten.
				</p>
			</div>
		</div>
	...
	{:else if anchors.length === 0}
		<div class="empty">
			...
			<p class="empty__sub">
				The integrity scheduler hasn't run for this database. See the <code>RUNBOOK</code> for
				the manual trigger command.
			</p>
		</div>
	{:else}
					...
					<tbody>
						{#each anchors as a (a.id)}
							<tr>
								...
								<td>
									<code class="hash" title={a.anchor_hash}
										>{a.anchor_hash.slice(0, 16)}…</code
									>
								</td>
	...
	{#if selected}
		...
		<aside class="drawer" aria-label="Integrity anchor">
			...
			<dl class="meta">
				...
				<dt>Anchored at</dt>
				<dd>{new Date(selected.anchored_at).toLocaleString()}</dd>
				<dt>Records</dt>
				<dd class="num">{selected.record_count.toLocaleString()}</dd>
				<dt>Window start</dt>
				<dd>
