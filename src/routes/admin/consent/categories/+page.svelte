...
<script lang="ts">
	...
	async function refresh() {
		...
		try {
			...
			error = e instanceof ApiError ? `${e.status}: ${e.message}` : 'Failed to load categories';
		} finally {
			loading = false;
		}
	}
		...
		<div class="page__title-row">
			...
			<div class="page__copy">
				...
				<p class="page__subtitle">
					Categories grouped under the consent banner. The <code>necessary</code> category is locked
					— it always reflects the strictly-required cookies and cannot be opted out of.
				</p>
			</div>
		</div>
					...
					<tbody>
						{#each categories as c (c.key)}
							...
							<tr>
								...
								<td>
									<span class={c.is_required ? 'pill pill--warn' : 'pill pill--neutral'}>
...
<style>
	...
	.desc { color: var(--color-grey-400); max-width: 32ch; overflow: hidden; text-overflow: ellipsis; }
	...
	.check-row { display: inline-flex; gap: 0.5rem; align-items: center; font-size: 0.875rem; color: var(--color-grey-200); cursor: pointer; padding-top: 1.5rem; }
