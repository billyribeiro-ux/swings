		...
		<div class="page__title-row">
			...
			<div class="page__copy">
				...
				<p class="page__subtitle">
					Transactional outbox (FDN-04). Each row is one domain event awaiting fan-out by
					the background dispatcher. Retry pulls a failed or dead-letter row back to <code
						>pending</code
					>
					...
					<tbody>
						{#each envelope.data as row (row.id)}
							<tr>
								...
								<td
									><span class={statusPill(String(row.status))}>{row.status}</span
									></td
								>
	...
	{#if selected}
		...
		<aside class="drawer" aria-label="Outbox event detail">
			...
			<dl class="meta">
				...
				<dt>Event type</dt>
				<dd><code>{selected.event_type}</code></dd>
				<dt>Aggregate</dt>
				<dd>
					<code>{selected.aggregate_type}</code> · <code>{selected.aggregate_id}</code>
				</dd>
				<dt>Status</dt>
				<dd><span class={statusPill(String(selected.status))}>{selected.status}</span></dd>
				<dt>Attempts</dt>
				<dd class="num">{selected.attempts} / {selected.max_attempts}</dd>
				<dt>Next attempt</dt>
				<dd>{new Date(selected.next_attempt_at).toLocaleString()}</dd>
				<dt>Created</dt>
				...
				{#if selected.last_error}
					<dt>Last error</dt>
					<dd class="err">{selected.last_error}</dd>
				{/if}
			</dl>
			<details open>
