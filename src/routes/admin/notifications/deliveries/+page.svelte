...
<script lang="ts">
	...
	async function refresh() {
		...
		try {
			...
			error =
				e instanceof ApiError ? `${e.status}: ${e.message}` : 'Failed to load deliveries';
		} finally {
			loading = false;
		}
	}
		...
		<div class="page__title-row">
			...
			<div class="page__copy">
				...
				<p class="page__subtitle">
					Every send the platform has attempted, with status, provider id, and rendered
					subject. Use this to triage "did the user get the email?" tickets.
				</p>
			</div>
		</div>
	</header>
							...
							<tr>
								...
								<td>
									...
									{:else if row.anonymous_email}
										<code title={row.anonymous_email}
											>{row.anonymous_email}</code
										>
									{:else}
								...
								<td>
									{#if row.provider_id}
										<code title={row.provider_id}
											>{row.provider_id.slice(0, 14)}…</code
										>
									{:else}
										<span class="muted">—</span>
									{/if}
	...
	{#if selected}
		...
		<aside class="drawer" aria-label="Delivery detail">
			...
			<dl class="meta">
				...
				<dt>Template</dt>
				<dd>
					<code>{selected.template_key}</code> ·
					<span class="muted-inline">{selected.channel}</span>
				</dd>
				<dt>Status</dt>
			...
			{#if selected.status === 'failed' || selected.status === 'bounced'}
				<p class="hint">
					Re-sending failed deliveries currently goes through the source channel — for
					transactional sends, retrigger the originating action; for marketing broadcasts,
					fix the suppression list and re-broadcast.
				</p>
			{/if}
		</aside>
	{/if}
