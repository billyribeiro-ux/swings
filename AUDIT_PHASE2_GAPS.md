...
| Webhook handlers with idempotency | Partial | `backend/src/handlers/webhooks.rs`, `migrations/017` | 55 | Partial | HMAC verify + idempotency table + 5-min tolerance + cleanup. Only handles 4 Stripe event types; no invoice.*, payment_intent.*, charge.*, dispute, dunning. Other providers: none. |
