...
> **Render is no longer supported.** The previous `render.yaml` blueprint
> and `backend/nixpacks.toml` have been removed. Railway is the canonical
> backend host (Vercel rewrites `/api/*` directly to the Railway service
> domain). If you need a second host for staging or DR, fork the
> Dockerfile path used by `backend/railway.toml` — it is the only
> Dockerfile in the repo and is consumed by both Railway and
> `docker-compose.yml`.
...
In your repo, `vercel.json` already proxies `/api/*` to the production
Railway domain via the `rewrites` block, and ships the production CSP /
HSTS / `X-Frame-Options` / `Permissions-Policy` headers under the
top-level `headers` block (these protect prerendered pages, which bypass
`hooks.server.ts` and would otherwise ship without CSP). If you forked
the repo or the Railway service hostname differs, update the rewrite
destination to match your service URL and commit.

> **Caveat — preview deploys hit production data.** Vercel's
> `vercel.json` does not support env-var interpolation in rewrite
> `destination` strings, and there is no first-class branch-conditional
> rewrite. That means every PR preview deploy proxies `/api/*` to the
> same production Railway service the main branch uses — preview
> traffic mutates production rows. Two pragmatic mitigations:
>
> 1. **Spin up a second Railway service** (`swings-staging`) and replace
>    the rewrite with a SvelteKit `+server.ts` proxy that reads
>    `process.env.VERCEL_ENV` (`"preview"` vs `"production"`) and
>    fans out to the right host. The proxy lives in
>    `src/routes/api/[...path]/+server.ts` and the rewrite is removed
>    from `vercel.json`.
> 2. **Disable previews entirely** in Vercel project settings until the
>    proxy is in place.
>
> This audit has _not_ added the proxy — it only documents the
> limitation. Track as a follow-up.
